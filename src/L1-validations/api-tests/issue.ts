...
export default function issue(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function ISSUE_CONTEXT(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
	...
	> - **condition B**: ["issue"] must **not** be equal to ["issue"]`,
        ...
        function ISSUE_ISSUE(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
                ...
                function ISSUE_ENUM(input: validationInput): validationOutput {
                    ...
                    for (const testObj of scope) {
                        ...
                        function Required_message_enum(
                            input: validationInput,
                        ): validationOutput {
                            const scope = payloadUtils.getJsonPath(
                                input.payload,
                                    ...
                                    return [
                                        {
                                            valid: false,
                                            code: 30000,
                                            description: `- **condition Required_message_enum**: every element of $.message.issue.status must be in ["OPEN", "CLOSED", "PROCESSING", "RESOLVED"]`,
                                        },
                                    ];
                                }
        ...
        function ISSUE_ACTORS(input: validationInput): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            let valid = true;
            for (const testObj of scope) {
  ...
  - **condition REQUIRED_MESSAGE_TYPE.2**: every element of $.message.issue.actors[*].type must be in ["INERFACING_NP", "COUNTERPARTY_NP", "CASCADED_NP", "PROVIDER", "AGENT", "CUSTOMER", "INTERFACING_NP_GRO", "COUNTERPARTY_NP_GRO", "CASCADED_NP_GRO"]`,
                ...
                function Actors_Attribute_Tests(
                    input: validationInput,
                ): validationOutput {
                    const scope = payloadUtils.getJsonPath(input.payload, "$");
                    let subResults: validationOutput = [];
