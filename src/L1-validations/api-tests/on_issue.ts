...
export default function on_issue(input: validationInput): validationOutput {
    ...
    for (const testObj of scope) {
        ...
        function REQUIRED_MESSAGE_STATUS(
            ...
            for (const testObj of scope) {
                ...
                const enumList = ["OPEN", "CLOSED", "PROCESSING", "RESOLVED"];
  ...
  - **condition REQUIRED_MESSAGE_NAME_30.2**: every element of $.message.issue.actions[*].descriptor.name must be in ["OPEN", "CLOSED", "PROCESSING", "RESOLVED", "INFO_REQUESTED", "INFO_PROVIDED", "INFO_NOT_AVAILABLE", "RESOLUTION_PROPOSED", "RESOLUTION_CASCADED", "RESOLUTION_ACCEPTED", "RESOLUTION_REJECTED"]`,
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition VALID_ENUM_MESSAGE_STATUS**: every element of $.message.issue.status must be in ["OPEN", "CLOSED", "PROCESSING", "RESOLVED"]
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition VALID_ENUM_MESSAGE_TYPE**: every element of $.message.issue.actors[*].type must be in ["INERFACING_NP", "COUNTERPARTY_NP", "CASCADED_NP", "PROVIDER", "AGENT", "CUSTOMER", "INTERFACING_NP_GRO", "COUNTERPARTY_NP_GRO", "CASCADED_NP_GRO"]
	...
	> - **condition B**: $.message.issue.actors[*].type must **not** be present in the payload`,
        ...
        function VALID_ENUM_MESSAGE_NAME(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition VALID_ENUM_MESSAGE_NAME**: every element of $.message.issue.actions[*].descriptor.name must be in ["OPEN", "CLOSED", "PROCESSING", "RESOLVED", "INFO_REQUESTED", "INFO_PROVIDED", "INFO_NOT_AVAILABLE", "RESOLUTION_PROPOSED", "RESOLUTION_CASCADED", "RESOLUTION_ACCEPTED", "RESOLUTION_REJECTED"]
	...
	> - **condition B**: $.message.issue.actions[*].descriptor.name must **not** be present in the payload`,
        ...
        function VALID_ENUM_MESSAGE_NAME_6(
            input: validationInput,
        ): validationOutput {
            const scope = payloadUtils.getJsonPath(input.payload, "$");
            let subResults: validationOutput = [];
            ...
            for (const testObj of scope) {
                ...
                const enumPath = payloadUtils.getJsonPath(
                    testObj,
                    "$.message.issue.resolutions[*].descriptor.name",
                );
                    ...
                    return [
                        {
                            valid: false,
                            code: 30000,
                            description: `- **condition VALID_ENUM_MESSAGE_NAME_6**: every element of $.message.issue.resolutions[*].descriptor.name must be in ["REFUND", "RETURN", "REPLACEMENT", "NO-ACTION", "CANCEL", "NOW-VISIBLE", "PARENT"]
	...
	> - **condition B**: $.message.issue.resolutions[*].descriptor.name must **not** be present in the payload`,
        ...
        const testFunctions: testFunctionArray = [
            ...
            VALID_ENUM_MESSAGE_NAME,
            VALID_ENUM_MESSAGE_NAME_6,
        ];
